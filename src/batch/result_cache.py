...
class ResultCache:
    ...
    def save_low_hunter_result(self, result_dict: Dict[str, Any]) -> bool:
        """
        Low Hunter（黄金の指値ボード）の結果を保存

        Args:
            result_dict: LowHunterPipeline.to_json_dict() の出力

        Returns:
            成功時True
        """
        low_hunter_dir = self.cache_dir / "low_hunter"
        low_hunter_dir.mkdir(parents=True, exist_ok=True)
        result_path = low_hunter_dir / "the_one_board.json"

        try:
            with open(result_path, 'w', encoding='utf-8') as f:
                json.dump(result_dict, f, ensure_ascii=False, indent=2)
            stock_count = len(result_dict.get('stocks', []))
            logger.info(f"Low Hunter結果保存完了: {stock_count}銘柄")
            return True
        except Exception as e:
            logger.error(f"Low Hunter結果保存エラー: {e}")
            return False

    def load_low_hunter_result(self) -> Optional[Dict[str, Any]]:
        """
        Low Hunter（黄金の指値ボード）の結果を読み込み

        Returns:
            結果の辞書。存在しない場合はNone。
        """
        result_path = self.cache_dir / "low_hunter" / "the_one_board.json"
    ...
    def save_pairs_result(self, result_dict: Dict[str, Any]) -> bool:
        """
        ペアトレード分析の結果を保存

        Args:
            result_dict: ペアトレード結果の辞書

        Returns:
            成功時True
        """
        pairs_dir = self.cache_dir / "pairs"
        pairs_dir.mkdir(parents=True, exist_ok=True)
        result_path = pairs_dir / "the_one_pairs.json"

        try:
            with open(result_path, 'w', encoding='utf-8') as f:
                json.dump(result_dict, f, ensure_ascii=False, indent=2)
            pair_count = len(result_dict.get('pairs', []))
            logger.info(f"ペアトレード結果保存完了: {pair_count}ペア")
            return True
        except Exception as e:
            logger.error(f"ペアトレード結果保存エラー: {e}")
            return False

    def load_pairs_result(self) -> Optional[Dict[str, Any]]:
        """
        ペアトレード分析の結果を読み込み
        ...
        Returns:
            結果の辞書。存在しない場合はNone。
        """
        result_path = self.cache_dir / "pairs" / "the_one_pairs.json"
        ...
        try:
            ...
            logger.error(f"ペアトレード結果読み込みエラー: {e}")
            return None
